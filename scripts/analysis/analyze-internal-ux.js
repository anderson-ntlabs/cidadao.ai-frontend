...
async function analyzeColorConsistency(page) {
  return await page.evaluate(() => {
    ...
    const grays = analysis.colorUsage.filter(
      (item) => item.color.includes('gray') || item.color.match(/rgb\((\d+), \1, \1\)/)
    )

    if (grays.length > 5) {
      analysis.recommendations.push(
        'Padronizar escala de cinzas - encontradas ' + grays.length + ' variações'
      )
    }
  ...
  return await page.evaluate(() => {
    ...
    if (buttonStyles.size > 5) {
      components.inconsistencies.push({
        ...
        recommendation:
          'Padronizar para 3-4 variantes máximo (primário, secundário, ghost, danger)',
      })
    }
...
async function analyzeSpacing(page) {
  return await page.evaluate(() => {
    ...
    if (spacings.size > 20) {
      analysis.recommendations.push(
        'Implementar escala de espaçamento consistente (ex: 4, 8, 16, 24, 32, 48px)'
      )
    }
