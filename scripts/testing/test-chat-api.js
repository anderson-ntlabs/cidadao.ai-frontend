...
async function runTests() {
  ...
  try {
    ...
    console.log(
      'Certifique-se de que o servidor está rodando com: cd ../cidadao.ai-backend && make run-dev'
    )
    return
  }

  // 2. Testar endpoints do chat
  await testEndpoint('Chat Message (Principal)', 'POST', '/api/v1/chat/message', {
    message: 'Olá, este é um teste',
  })

  await testEndpoint('Chat Suggestions', 'GET', '/api/v1/chat/suggestions')

  await testEndpoint('Lista de Agentes', 'GET', '/api/v1/agents')

  await testEndpoint('Chat History', 'GET', '/api/v1/chat/history/test-session-123')
  ...
  await testEndpoint('API Docs', 'GET', '/docs')
