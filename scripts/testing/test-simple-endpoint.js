...
async function testSimpleEndpoint() {
  ...
  console.log(
    `Taxa de Sucesso: ${successCount}/${testCases.length} (${((successCount / testCases.length) * 100).toFixed(1)}%)`
  )
  ...
  if (successfulResults.length > 0) {
    ...
    const avgLength =
      successfulResults.reduce((sum, r) => sum + r.responseLength, 0) / successfulResults.length
