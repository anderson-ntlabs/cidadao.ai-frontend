...
async function testDrummondStress() {
  ...
  console.log(
    `✅ Sucessos (Drummond): ${results.success} (${((results.success / results.total) * 100).toFixed(1)}%)`
  )
  console.log(
    `⚠️  Manutenção: ${results.maintenance} (${((results.maintenance / results.total) * 100).toFixed(1)}%)`
  )
  console.log(
    `❌ Undefined: ${results.undefined} (${((results.undefined / results.total) * 100).toFixed(1)}%)`
  )
  console.log(
    `❌ Erros: ${results.errors} (${((results.errors / results.total) * 100).toFixed(1)}%)`
  )
