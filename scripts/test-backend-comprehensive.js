...
function generateReport() {
  ...
  log(`   ✅ Passed: ${results.passed} (${Math.round(results.passed/results.total*100)}%)`, colors.green);
  log(`   ❌ Failed: ${results.failed} (${Math.round(results.failed/results.total*100)}%)`, colors.red);
  ...
  const criticalErrors = results.errors.filter(e =>
    e.test.includes('TC001') || e.test.includes('TC002') || e.test.includes('TC003')
  );
