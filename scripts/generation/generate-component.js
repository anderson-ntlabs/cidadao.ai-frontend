...
if (!/^[A-Z][a-zA-Z]*$/.test(componentName)) {
  console.error(
    `${colors.red}Error: Component name must be in PascalCase (e.g., MyComponent)${colors.reset}`
  )
  process.exit(1)
}
...
try {
  // Component file
  fs.writeFileSync(path.join(componentDir, `${kebabCase}.tsx`), componentTemplate)

  // Test file
  fs.writeFileSync(path.join(componentDir, `${kebabCase}.test.tsx`), testTemplate)
