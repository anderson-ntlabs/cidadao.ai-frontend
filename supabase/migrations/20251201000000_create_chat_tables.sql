-- Migration: Create chat tables (investigations, chat_sessions) with proper structure
-- Author: Anderson Henrique da Silva
-- Date: 2025-12-01
-- Purpose: Enable chat persistence to Supabase

-- ============================================================================
-- Table: investigations
-- Description: Stores user investigations for chat context
-- ============================================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tablename = 'investigations' AND policyname = 'Users can view own investigations'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'investigations' AND policyname = 'Users can create investigations'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'investigations' AND policyname = 'Users can delete own investigations'
    ) THEN
...
-- ============================================================================
-- Table: chat_sessions
-- Description: Stores chat sessions with messages as JSONB
-- Important: Includes session_id column required by ChatSessionService
-- ============================================================================
CREATE TABLE IF NOT EXISTS public.chat_sessions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id TEXT UNIQUE NOT NULL, -- Custom session ID from frontend (required by service)
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'chat_sessions' AND policyname = 'Users can create chat sessions'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'chat_sessions' AND policyname = 'Users can update own chat sessions'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'chat_sessions' AND policyname = 'Users can delete own chat sessions'
    ) THEN
...
-- Comments
-- ============================================================================
COMMENT ON TABLE public.investigations IS 'Stores user investigations for organizing chat sessions and tracking research';
COMMENT ON TABLE public.chat_sessions IS 'Stores chat sessions with messages, linked to investigations and agents';
COMMENT ON COLUMN public.chat_sessions.session_id IS 'Frontend-generated unique session identifier';
COMMENT ON COLUMN public.chat_sessions.messages IS 'Array of chat messages stored as JSONB';
