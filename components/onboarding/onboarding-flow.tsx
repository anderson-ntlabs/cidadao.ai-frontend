...
const onboardingSteps: OnboardingStep[] = [
  {
    ...
    description:
      'Sua plataforma de transparência pública com inteligência artificial. Vamos configurar sua conta em poucos passos.',
  ...
  },
  {
    ...
    description:
      'Visualize dados em tempo real, acompanhe investigações e exporte relatórios detalhados.',
    icon: (
  ...
  },
  {
    ...
    description:
      'Configure notificações para receber alertas sobre anomalias detectadas e novas investigações.',
    action: {
      label: 'Configurar Notificações',
      onClick: () => logger.info('Configure notifications action triggered'),
    },
  },
  {
    id: 'ready',
    title: 'Tudo Pronto!',
    description:
      'Você está pronto para começar a usar o Cidadão.AI. Explore o dashboard ou inicie uma conversa com nossos agentes.',
      ...
      <div className="w-full max-w-2xl mx-4">
        <div className="bg-card rounded-lg shadow-xl p-8">
          ...
          <div className="min-h-[400px] flex flex-col items-center justify-center text-center animate-fade-in-up">
            ...
            {step.icon && !step.image && <div className="mb-6">{step.icon}</div>}

            <h2 className="text-3xl font-bold mb-4">{step.title}</h2>
            <p className="text-lg text-muted-foreground mb-8 max-w-md">{step.description}</p>

            {step.action && (
              <Button onClick={step.action.onClick} variant="secondary" className="mb-4">
                {step.action.label}
              </Button>
            )}
