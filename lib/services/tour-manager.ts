...
export class TourManager {
  ...
  initialize(config: Partial<TourConfig>) {
    ...
    this.driver = driver({
      animate: true,
      // opacity: 0.75, // Not supported in new API
      // padding: 10,
      allowClose: true,
      ...
      onDestroyed: () => {
        this.analytics.track('tour_completed', {
          ...
          steps_viewed: this.getCurrentStep()
        });
      }
    });
  }
    ...
    return [
      {
        ...
        popover: {
          ...
          description: 'Digite sua pergunta sobre transparência pública. Por exemplo: "Como renovar meu RG?"',
          position: 'top'
        }
      },
      {
