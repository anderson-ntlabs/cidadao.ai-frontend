...
function sendToAnalytics(metric: WebVitalsMetric) {
  ...
  // Also log to logger in development
...
function getRecommendations(vitals: Record<string, WebVitalsMetric>): string[] {
  ...
  if (vitals.LCP?.rating !== 'good') {
    recommendations.push(
      'Optimize largest content paint by lazy loading images and improving server response times'
    )
  }

  if (vitals.CLS?.rating !== 'good') {
    recommendations.push(
      'Reduce layout shifts by setting explicit dimensions for images and dynamic content'
    )
  }

  if (vitals.FID?.rating !== 'good' || vitals.INP?.rating !== 'good') {
    recommendations.push(
      'Improve interactivity by reducing JavaScript execution time and using web workers'
    )
  }
