...
const logger = createLogger('AgoraDemo')

/**
 * Ágora Demo Mode Hook
 *
 * MVP mode - no real auth, all data in localStorage
 * Allows anyone to explore the Ágora without authentication
 */

const STORAGE_KEY = 'agora_demo_user'
const STORAGE_XP_KEY = 'agora_demo_xp_transactions'
const STORAGE_DIARY_KEY = 'agora_demo_diary'
const STORAGE_SESSIONS_KEY = 'agora_demo_sessions'
const STORAGE_LGPD_KEY = 'agora_demo_lgpd_consent'
const STORAGE_CONTRACT_KEY = 'agora_demo_internship_contract'
const STORAGE_BADGES_KEY = 'agora_demo_badges'
const STORAGE_ONBOARDING_KEY = 'agora_demo_onboarding'

export type AgoraTrack = 'backend' | 'frontend' | 'ia' | 'devops'

export interface AgoraDemoUser {
  id: string
  name: string
  email: string
  avatar: string
  ...
  currentLevel: number
  currentRank: string
  tracks: AgoraTrack[] // User can have multiple tracks
...
export interface OnboardingData {
  ...
  selectedTracks: AgoraTrack[] // User can select multiple tracks
...
const DEFAULT_DEMO_USER: AgoraDemoUser = {
  id: 'demo-user-001',
  name: 'Estudante Demo',
...
interface AgoraDemoContextType {
  ...
  // Actions
  updateProfile: (updates: Partial<AgoraDemoUser>) => void
  ...
  toggleTrack: (track: AgoraTrack) => void // Toggle track selection (add/remove)
...
const AgoraDemoContext = createContext<AgoraDemoContextType | undefined>(undefined)

export function AgoraDemoProvider({ children }: { children: React.ReactNode }) {
  const [user, setUser] = useState<AgoraDemoUser>(DEFAULT_DEMO_USER)
  ...
  const updateProfile = useCallback((updates: Partial<AgoraDemoUser>) => {
    setUser((prev) => {
  ...
  const checkAndAwardBadges = useCallback(() => {
    const newBadges: AgoraBadge[] = [...badges]
    ...
    if (!hasJapaguri && qualifiesForJapaguri) {
      const japaguriBadge: AgoraBadge = {
    ...
    if (!hasPioneiro && user.hasAcceptedInternshipContract) {
      const pioneiroBadge: AgoraBadge = {
        id: `badge_pioneiro_${Date.now()}`,
        type: 'pioneiro',
        name: 'Pioneiro',
        description: 'Um dos primeiros a embarcar na jornada da Ágora!',
        emoji: '🚀',
        earnedAt: new Date().toISOString(),
    ...
    if (!hasExplorador && uniqueAgentsUsed >= 3) {
      const exploradorBadge: AgoraBadge = {
    ...
    if (!hasDedicado && qualifiesForDedicado) {
      const dedicadoBadge: AgoraBadge = {
  ...
  const completeOnboarding = useCallback(() => {
    ...
    addXp(100, 'onboarding', 'Onboarding concluído! Bem-vindo à Ágora!')
...
export function useAgoraDemo() {
  const context = useContext(AgoraDemoContext)
  if (context === undefined) {
    throw new Error('useAgoraDemo must be used within an AgoraDemoProvider')
  }
  return context
}
