...
test.describe('CardV2 Component', () => {
  ...
  test.describe('Interaction Tests', () => {
    test('should show hover effects on interactive cards', async ({ page }) => {
      ...
      const initialTransform = await interactiveCard.evaluate(el => 
      ...
      // Check lift effect
      const hoverTransform = await interactiveCard.evaluate(el => 
        window.getComputedStyle(el).transform
      );
    ...
    test('should show hover border change on outlined variant', async ({ page }) => {
      ...
      const initialBorderColor = await outlinedCard.evaluate(el => 
      ...
      const hoverBorderColor = await outlinedCard.evaluate(el => 
        window.getComputedStyle(el).borderColor
      );
  ...
  test.describe('Composition Tests', () => {
    test('should render all card sub-components correctly', async ({ page }) => {
      ...
      // Check description
      await expect(cardWithFooter.locator('text=Análise detalhada de gastos públicos')).toBeVisible();
  ...
  test.describe('Responsive Tests', () => {
    ...
    test('should maintain proper layout on tablet', async ({ page }) => {
      ...
      // Check grid layout
      const statCards = page.locator('text=Cards de Estatísticas').locator('..//div[contains(@class, "grid")]');
  ...
  test.describe('Dark Mode Tests', () => {
    ...
    test('should display correct colors in dark mode', async ({ page }) => {
      ...
      const bgColor = await elevatedCard.evaluate(el => 
        window.getComputedStyle(el).backgroundColor
      );
