...
test.describe('Chat Flow', () => {
  ...
  test('should send and receive messages', async ({ page }) => {
    ...
    await expect(page.locator('text="Olá, preciso de ajuda com transparência pública"')).toBeVisible();
  ...
  test('should display suggested actions', async ({ page }) => {
    // Send a message
    await page.locator('textarea[placeholder*="Digite sua mensagem"]').fill('O que você pode fazer?');
    await page.locator('button[aria-label*="Enviar"]').click();
  ...
  test('should preserve chat session on page reload', async ({ page }) => {
    // Send a message
    await page.locator('textarea[placeholder*="Digite sua mensagem"]').fill('Mensagem antes do reload');
    await page.locator('button[aria-label*="Enviar"]').click();
