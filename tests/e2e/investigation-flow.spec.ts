...
test.describe('Investigation Flow', () => {
  ...
  test('should sort investigations', async ({ page }) => {
    ...
    if (await sortButton.isVisible()) {
      ...
      if (count >= 2) {
        const firstDate = await cards.first().locator('[data-testid="investigation-date"]').textContent();
        const lastDate = await cards.last().locator('[data-testid="investigation-date"]').textContent();
