...
const response = await smartChatService.sendMessage('Olá, como funciona a transparência pública?')
...
```typescript
const response = await smartChatService.sendMessage('Analise os contratos do governo federal', {
  streaming: true,
  onChunk: (text) => {
...
```typescript
const response = await smartChatService.sendMessage('O que é Portal da Transparência?', {
...
const response = await smartChatService.sendMessage('Quem são os agentes?')
...
```typescript
const updated = await investigationService.updateInvestigation('investigation_id_123', {
  status: 'completed',
  metadata: {
    findings: 15,
    anomalies: 3,
...
function isChatResponse(obj: any): obj is ChatResponse {
  return typeof obj === 'object' && 'session_id' in obj && 'agent_id' in obj && 'message' in obj
}
...
// Usage
const response = await retryWithBackoff(() => api.post('/api/v1/chat/message', request), 3, 1000)
```
...
const response = await breaker.execute(() => api.post('/api/v1/chat/message', request))
```
...
4. **Implement Cache Warming**:
   ...
   useEffect(() => {
     Promise.all([smartChatService.sendMessage('warm cache'), fetchTransparencyMap()])
   }, [])
   ```
...
describe('Chat API Integration', () => {
  it('should send message and receive response', async () => {
    const response = await smartChatService.sendMessage('Olá, como você funciona?')
  ...
  it('should handle streaming responses', async () => {
    ...
    const response = await smartChatService.sendMessage('Explique o Portal da Transparência', {
      streaming: true,
...
```typescript
const response = await smartChatService.sendMessage('Analyze all government contracts from 2024', {
  streaming: true,
  onProgress: (text) => {
...
// Usage
const agents = await deduplicatedRequest('agents', () => api.get('/api/v1/agents'))
```
