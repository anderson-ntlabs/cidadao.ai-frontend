...
const investigationSessions = await chatSessionService.getInvestigationSessions('inv_456')
```
...
```typescript
const MODEL_COSTS = {
  'sabiazinho-3': 0.2, // per 1M tokens
  'sabia-3': 0.6, // per 1M tokens
  mixed: 0.4, // average
  fallback: 0.0, // local only
}
```
