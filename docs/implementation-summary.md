...
```typescript
const { investigation, results, startPolling, stopPolling } =
  useBackendInvestigation({ investigationId: 'uuid', autoPoll: true });
```

#### `useCreateBackendInvestigation()`
Cria e rastreia nova investigação automaticamente
```typescript
const { createAndTrack, investigation, isPolling } =
  useCreateBackendInvestigation();
```
...
const sortedInvestigations = useMemo(() => {
  return [...filteredInvestigations].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'anomalies': return (b.anomalies_detected || 0) - (a.anomalies_detected || 0);
