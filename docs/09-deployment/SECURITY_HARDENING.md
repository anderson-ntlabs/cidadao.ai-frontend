...
const { field, order } = validateSort(request.query.sort || 'createdAt:desc', [
  'createdAt',
  'updatedAt',
  'name',
])
...
export async function POST(request: NextRequest) {
  try {
    ...
    validator
      ...
      .custom('agentId', () => isValidAgentId(body.agentId), 'Invalid agent ID')
...
export async function POST(request: NextRequest) {
  ...
  validator.required('data', body.data).length('data', body.data, 1, 1000)
