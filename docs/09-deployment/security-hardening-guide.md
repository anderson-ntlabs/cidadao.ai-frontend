...
export async function middleware(request: NextRequest) {
  ...
  const simpleCsp =
    "default-src 'self' https:; frame-src 'self' https://open.spotify.com https://vlibras.gov.br; ..."
...
const { data } = await supabase.from('investigations').select('*').eq('user_id', userId) // Parameterized, not concatenated
