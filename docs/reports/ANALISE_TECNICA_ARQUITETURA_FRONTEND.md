...
class SmartChatService {
  ...
  selectEndpoints(message, options: {
...
class MLAdapterSelector {
  ...
  private trainModel() {
    ...
    const labels = this.telemetryData.map(metric =>
      metric.bestAdapter  // adapter que teve melhor performance
    );
...
*Este documento é confidencial e destinado exclusivamente para uso interno do projeto Cidadão.AI. A reprodução ou distribuição não autorizada é proibida.*
