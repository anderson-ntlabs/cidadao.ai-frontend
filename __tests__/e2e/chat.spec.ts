...
test.describe('Chat Interaction', () => {
  ...
  test('should clear input after sending message', async ({ page }) => {
    ...
    if (await sendButton.isVisible()) {
      ...
      const hasMessage = await page.getByText('Teste de mensagem').isVisible().catch(() => false);
  ...
  test('should handle keyboard shortcuts', async ({ page }) => {
    ...
    const hasLoadingIndicator = await page.getByText(/enviando|processando|loading/i).isVisible().catch(() => false);
    const hasMessage = await page.getByText('Mensagem de teste').isVisible().catch(() => false);
  ...
  test('should display agent information when present', async ({ page }) => {
    ...
    if (await chatInput.isVisible()) {
      ...
      if (await sendButton.isVisible()) {
        ...
        const hasAgentName = await page.getByText(/Abaporu|Zumbi|Anita|Tiradentes/i).isVisible().catch(() => false);
